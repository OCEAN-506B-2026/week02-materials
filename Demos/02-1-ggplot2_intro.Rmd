---
title: "ggplot2 demo: introduction"
author: "Wing-Ho Ko"
...
Figure with two data series from the same source: plot the engine displacement in liter (`displ`) against both highway and city mile per gallon
...
To make legend appears in the above figure, use the "constant aesthetics" trick:
...
Figure with data from two sources: plot the `mpg` observations (1999-2008) against the data from the `mtcars` observations (1973-1974)

```{r}
ggplot() + 
  geom_point(aes(x=displ, y=hwy, color="1999-2008"), mpg) +
  geom_point(aes(x=0.0163871 * disp, y=mpg, color="1973-1974"), mtcars)
```
